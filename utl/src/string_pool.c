...
#define UTL_STRING_POOL_OBJECTS_PER_PAGE 8
...
static inline void utl_StringPool_maybe_alloc_new_page(utl_StringPoolPageBase** current_page_p, utl_StringPoolObjectBase** free_list_p, size_t object_size) {
    ...
    current_page = malloc(sizeof(utl_StringPoolPageBase) + object_size * UTL_STRING_POOL_OBJECTS_PER_PAGE);
    current_page->max_size = object_size - sizeof(utl_StringPoolObjectBase);
    current_page->free_objects = UTL_STRING_POOL_OBJECTS_PER_PAGE;
    current_page->next = next_page;
    current_page->objects = (utl_StringPoolObjectBase*)current_page + 1;

    for (size_t i = 0; i < UTL_STRING_POOL_OBJECTS_PER_PAGE; ++i) {
...
void utl_StringPool_free(utl_StringView string) {
    utl_StringPoolObjectBase* obj = (utl_StringPoolObjectBase*)string.data - 1;
    ++obj->page->free_objects;

    utl_StringPoolObjectBase* free_list = NULL;
    switch (obj->page->max_size + sizeof(utl_StringPoolObjectBase)) {
...
utl_StringView utl_StringPool_realloc(utl_StringView string, size_t size) {
    ...
    const utl_StringPoolObjectSmall* obj = (utl_StringPoolObjectSmall*)string.data - 1;
